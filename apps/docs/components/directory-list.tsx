...
import { DirectoryAddButton, DirectoryAddProvider } from '@/components/directory-add-button'
...
function getPageNumbers(current: number, total: number) {
  ...
  if (current <= 3) {
    for (let page = 2; page <= 4; page++) pages.push(page)
    pages.push('ellipsis', total)
  } else if (current >= total - 2) {
    pages.push('ellipsis')
    for (let page = total - 3; page <= total; page++) pages.push(page)
  } else {
    pages.push('ellipsis')
    for (let page = current - 1; page <= current + 1; page++) pages.push(page)
    pages.push('ellipsis', total)
  }
...
function DirectoryListContent() {
  ...
  const { isLoading, paginatedRegistries, page, query, registries, totalPages, setPage, setQuery } =
    useSearchRegistry()
  const previousHref = page > 1 ? getPageHref(pathname, query, page - 1) : undefined
  const nextHref = page < totalPages ? getPageHref(pathname, query, page + 1) : undefined

  const handlePageChange = React.useCallback(
    (event: React.MouseEvent<HTMLAnchorElement>, targetPage: number, disabled = false) => {
      if (
        ...
        {paginatedRegistries.map((registry, index) => (
          <React.Fragment key={registry.name}>
            <Item className="group/item relative gap-6 px-0">
              <ItemMedia
                variant="image"
                // biome-ignore lint/security/noDangerouslySetInnerHtml: SVG logo from registry config
            ...
            {index < paginatedRegistries.length - 1 && <ItemSeparator className="my-1" />}
          </React.Fragment>
        ))}
      </ItemGroup>
      {totalPages > 1 && (
          ...
          <PaginationContent>
            <PaginationItem>
              <DirectoryPaginationPrevious
                ...
                onClick={(event) => handlePageChange(event, page - 1, page <= 1)}
                className={cn(page <= 1 ? 'pointer-events-none opacity-50' : 'cursor-pointer')}
              />
            </PaginationItem>
            {getPageNumbers(page, totalPages).map((p, i, pages) =>
              p === 'ellipsis' ? (
            ...
            <PaginationItem>
              <DirectoryPaginationNext
                ...
                onClick={(event) => handlePageChange(event, page + 1, page >= totalPages)}
                className={cn(
