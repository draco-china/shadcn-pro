          ...
          <pre className="no-scrollbar max-h-[300px] min-w-0 overflow-x-auto px-4 py-3.5 outline-none has-[[data-highlighted-line]]:px-0 has-[[data-line-numbers]]:px-0 has-[[data-slot=tabs]]:p-0 md:max-h-[450px]">
            ...
            <code data-line-numbers data-language="css">
              ...
              {Object.entries(activeTheme?.cssVars.light || {}).map(([key, value]) => (
                <span data-line className="line text-code-foreground" key={key}>
                  &nbsp;&nbsp;&nbsp;--{key}: <ColorIndicator color={`hsl(${value})`} /> hsl({value}
                  );
                </span>
              ))}
              <span data-line className="line text-code-foreground">
...
function ColorIndicator({ color }: { color: string }) {
  return (
    <span
      aria-hidden="true"
      className="inline-block size-2 rounded-full border align-middle"
      style={{ backgroundColor: color }}
    />
  )
}

function getThemeCodeOKLCH(theme: ThemeCssVars | undefined, radius: number) {
  if (!theme) return ''
  ...
  return [
    ...
    '',
    '.dark {',
    ...Object.entries(theme.dark).map(([key, value]) => `  --${key}: ${value};`),
    '}',
  ].join('\n')
}

function getThemeCodeHSLV4(theme: { cssVars: ThemeCssVars } | undefined, radius: number) {
  return getThemeCodeOKLCH(theme?.cssVars, radius)
}

function getThemeCode(theme: { cssVars: ThemeCssVars } | undefined, radius: number) {
  return getThemeCodeOKLCH(theme?.cssVars, radius)
}
