...
export function DirectoryAddButton({ registry }: { registry: { name: string } }) {
  ...
  return (
    <Button size="sm" variant="outline" className="relative z-10" onClick={() => open(registry)}>
      Add <Plus />
    </Button>
  )
}

export function DirectoryAddProvider({ children }: { children: React.ReactNode }) {
      ...
      {isMobile ? (
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogContent className="animate-none! sm:max-w-md">
            ...
            {Content}
            <DialogFooter>
              <DialogClose render={<Button size="sm" variant="outline" />}>Done</DialogClose>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      )}
