            ...
            <div key={option.value} className="flex items-start gap-2">
              <CheckboxControl
                ...
                onCheckedChange={(nextChecked) => {
                  if (nextChecked === true) {
                    if (!valueSet.has(option.value)) commit([...values, option.value])
                    return
                  }
...
> & {
  ...
  return (
    <SwitchPrimitive.Root
      ...
      className={cn(
        'peer inline-flex h-[1.15rem] w-8 shrink-0 items-center rounded-full border border-transparent shadow-xs transition-all outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 data-[state=checked]:bg-primary data-[state=unchecked]:bg-input dark:aria-invalid:ring-destructive/40 dark:data-[state=unchecked]:bg-input/80',
        className,
      )}
      {...props}
    >
