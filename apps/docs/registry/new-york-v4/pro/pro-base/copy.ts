...
export type ProCopyStatus = 'idle' | 'copying' | 'success' | 'error'

export interface ProCopyOptions {
...
export function useProCopy(options?: ProCopyOptions) {
  const [status, setStatus] = React.useState<ProCopyStatus>('idle')

  React.useEffect(() => {
    const resetDuration = options?.resetDuration ?? 2000
    if ((status !== 'success' && status !== 'error') || resetDuration <= 0) return
    const timer = window.setTimeout(() => setStatus('idle'), resetDuration)
    return () => window.clearTimeout(timer)
  }, [status, options?.resetDuration])

  const copy = React.useCallback(async () => {
    if (!options || status === 'copying') return
