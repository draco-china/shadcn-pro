...
/** Accessible image lightbox with navigation and transforms. */
export function ImageViewer({
  ...
  }, [open, index, list.length, controlledIndex, onIndexChange])
  ...
  const content = (
    <DialogPrimitive.Root open={open} onOpenChange={(nextOpen) => !nextOpen && onClose()}>
      <DialogPrimitive.Content
        ref={fullscreen.ref}
        className={cn(
          'fixed inset-0 z-50 flex flex-col bg-background/95 text-foreground outline-none',
          className,
        )}
        onWheel={(event) => {
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-2 px-4 py-3 text-foreground">
          ...
          <div className="flex justify-end">
            <DialogPrimitive.Close asChild>
              <ProButton size="icon" variant="ghost" tooltip="Close image viewer">
                <X />
              </ProButton>
...
function getPortalTarget(container: Element | DocumentFragment | null | undefined) {
